...
def contrail_analytics_joined(rid=None):
    ip_list = leader_get("cluster_info").values()
    if len(common_utils.json_loads(leader_get("cluster_info").values(), list())) < config.get("min-cluster-size"):
        ip_list = '[]'
...
def _address_changed(unit, ip):
    ips = common_utils.json_loads(leader_get("cluster_info"), dict())
    if ip in ip.values():
        return
    ips[unit] = ip
...
def leader_elected():
    ip = common_utils.get_ip()
    ip_list = common_utils.json_loads(leader_get("cluster_info").values(), list())
