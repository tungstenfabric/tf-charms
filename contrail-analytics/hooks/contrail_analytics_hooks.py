...
def leader_elected():
    ip = common_utils.get_ip()
    ip_list = common_utils.json_loads(leader_get("analytics_{}_list".format("ip")), list())
    ips = utils.get_analytics_ips("unit-address", ip)
    if not ip_list:
        ip_list = ips.values()
        log("IP_LIST: {}    IPS: {}".format("IP", str(ip_list), "IP", str(ips)))
        settings = {
            "analytics_ip_list": json.dumps(list(ip_list)),
            "analytics_ips".format("ip"): json.dumps(ips)
        }
        leader_set(settings=settings)
    else:
