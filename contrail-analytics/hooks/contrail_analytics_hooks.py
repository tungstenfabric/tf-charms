...
def _address_changed(unit, ip):
    ip_list = common_utils.json_loads(leader_get("cluster_ip_list"), list())
    ips = common_utils.json_loads(leader_get("cluster_ips"), dict())
    if ip in ip_list:
        return
    old_ip = ips.get(unit)
    if old_ip:
    ...
    settings = {
        "cluster_ip_list": json.dumps(ip_list),
        "cluster_ips": json.dumps(ips)
    }
...
def leader_elected():
    ...
    if not ip_list:
        ...
        log("IP_LIST: {}    IPS: {}".format(str(ip_list), str(ips)))
        settings = {
            "cluster_ip_list": json.dumps(list(ip_list)),
            "cluster_ips": json.dumps(ips)
        }
        leader_set(settings=settings)
    else:
