...
def contrail_analytics_joined(rid):
...
def contrail_analyticsdb_joined(rid):
    ip_list = leader_get("analytics_ip_list")
    if len(common_utils.json_loads(leader_get("analytics_ip_list"), list())) < config.get("min-cluster-size"):
        ip_list = '[]'
...
def _address_changed(unit, ip):
    ip_list = common_utils.json_loads(leader_get("analytics_ip_list"), list())
    ips = common_utils.json_loads(leader_get("analytics_ips"), dict())
    if ip in ip_list:
        return
    old_ip = ips.get(unit)
    if old_ip:
        ...
        ip_list.append(ip)
        ips[unit] = ip
    log("~~~ _address_changed({}, {})".format(unit, ip))
    log("IP_LIST: {}    IPS: {}".format(str(ip_list), str(ips)))
    settings = {
        "analytics_ip_list": json.dumps(ip_list),
        "analytics_ips": json.dumps(ips)
    }
...
def update_relations(rid=None):
    for rid in relation_ids("contrail-analytics"):
        contrail_analytics_joined(rid)
    for rid in relation_ids("contrail-analyticsdb"):
        contrail_analyticsdb_joined(rid)
...
def leader_elected():
    ip = common_utils.get_ip()
    ip_list = common_utils.json_loads(leader_get("analytics_ip_list"), list())
    ips = utils.get_analytics_ips("private-address", ip)
    if not ip_list:
        ip_list = ips.values()
        log("~~~ analytics' leader-elected, ip_list = ips.values()")
        log("IP_LIST: {}    IPS: {}".format(str(ip_list), str(ips)))
        settings = {
            "analytics_ip_list": json.dumps(list(ip_list)),
            "analytics_ips": json.dumps(ips)
        }
        leader_set(settings=settings)
    else:
        log("~~~ analytics' leader-elected, ip_list = common_utils.json_loads(leader_get...)")
        log("IP_LIST: {}    IPS: {}".format(str(ip_list), str(ips)))
