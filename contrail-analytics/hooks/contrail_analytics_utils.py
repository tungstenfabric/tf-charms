...
def get_analytics_ips(address_type, own_ip):
    ...
    for rid in relation_ids("analytics-cluster"):
        for unit in related_units(rid):
            analytics_ips[unit] = relation_get(address_type, unit, rid)
    log("~~~ get_analytics_ips' prepared dict(): {}".format(json.dumps(analytics_ips)))
    # add it's own ip address
    log("~~~ analytics_ips[local_unit()] = own_ip  -->  {}".format(json.dumps({local_unit(): own_ip})))
    analytics_ips[local_unit()] = own_ip
    log("~~~ result analytics_ips dict(): {}".format(json.dumps(analytics_ips)))
