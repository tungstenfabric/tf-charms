...
###EDIT###
    ...
    if ctx["contrail_version"] > 500 and ctx.get("analyticsdb_enabled"):
        for image in images:
...
def _update_charm_status(ctx):
    ...
    ###EDIT###
    if ctx["contrail_version"] == 500 and not ctx.get("analyticsdb_servers"):
    ...
    if ctx["contrail_version"] > 500 and not ctx.get("analyticsdb_enabled"):
...
def ziu_stage_1(ziu_stage, trigger):
    ...
    ###EDIT###
    if cver >= 510 and not analyticsdb_enabled():
