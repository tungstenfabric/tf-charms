...
###EDIT###
    ...
    if ctx.get("analyticsdb_enabled"):
        for image in images:
            if image == "contrail-analytics-alarm-gen":
                images.remove("contrail-analytics-alarm-gen")
            if image == "contrail-analytics-snmp-collector":
                images.renove("contrail-analytics-snmp-collector")
            if image == "contrail-analytics-snmp-topology":
                images.remove("contrail-analytics-snmp-collector")
            if image == "contrail-analytics-topology":
                images.remove("contrail-analytics-topology")
    for image in images:
        try:
...
def _update_charm_status(ctx):
    ...
    services = SERVICES.get(ctx["contrail_version"], SERVICES.get(9999))
    if not ctx.get("analyticsdb_enabled"):
        services.pop("analytics-alarm")
        services.pop("analytics-snmp")
        for service in services.keys():
            service_list = services.get(service)
            for item in service_list:
                if item == "topology":
                    service_list.remove("topology")
                if item == "alarm-gen":
                    service_list.remove("alarm-gen")
            services.update({service: service_list})
...
def ziu_stage_1(ziu_stage, trigger):
    ...
    ###EDIT###
    if cver >= 510 and analyticsdb_enabled():
