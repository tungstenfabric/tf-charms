...
def get_analytics_ips(address_type, own_ip):
    ...
    log("~~~ get_analytics_ips' prepared dict(): {}".format(json.dump(analytics_ips)))
    # add it's own ip address
    log("~~~ analytics_ips[local_unit()] = own_ip  -->  {}".format(json.dump({local_unit(): own_ip})))
    analytics_ips[local_unit()] = own_ip
    log("~~~ result analytics_ips dict(): {}".format(json.dump(analytics_ips)))
