...
def _notify_heat(rid=None):
    ...
    for rid in ([rid] if rid else relation_ids("heat-plugin")):
        relation_set(relation_id=rid, relation_settings=settings)
...
def _notify_neutron(rid=None):
    ...
    for rid in ([rid] if rid else relation_ids("neutron-api")):
        relation_set(relation_id=rid, relation_settings=settings)
...
def _notify_nova(rid=None):
    ...
    settings = {
        ...
        "subordinate_configuration": json.dumps(conf)}
    for rid in ([rid] if rid else relation_ids("nova-compute")):
        relation_set(relation_id=rid, relation_settings=settings)
